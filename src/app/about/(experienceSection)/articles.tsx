import { ExperienceArticle as Article } from "@/components/molecular/articles/experienceArticle";
...
const ImqaArticle = (props) => {
  return (
    <Article
      ...
      contribution={"프로젝트 전체 기여도 - 30% / 프론트엔드 기여도 - 30 %"}
      content={
        <>
          - 웹/모바일 성능 모니터링 서비스인 [IMQA](https://imqa.io/)의
          프론트엔드 파트를 담당해서 신규 기능 개발 및 유지보수 작업을 함
          <br />
          ...
          화를 통해 한 곳에서 관리할 수 있도록 작업 - Next.js 마이그레이션과
          함께, 디자인팀과의 협업을 통해 Atomic Design System 도입
          <br />
          ...
          1.4s 로 개선, 번들 사이즈 축소 (Node.js
          Compression,brotliCompress,Code spliting,Lazy Loading)
          <br />
      ...
      accordionContent={
        <>
          ...
          규칙을 서로 정하고 의견을 통일시켜 불필요한 커뮤니케이션을 줄임 - 오픈
          소스 라이브러리에서 원하는 기능을 제공 하지 않음 → 소스 코드 분석후
          해당 기능 직접 제작
        </>
      }
    ></Article>
  ...
  return (
    <Article
      ...
      content={
        <>
          <div>
            - 서비스 기획/설계/구현 모두 참여: - 사용자 중심의 사고 방식으로
            기존 서비스의 단점을 개선 하고, 장점을 살리는 방향으로 서비스
            기획/설계에 적극 참여
            <br />
            ...
            접근가능하게 함.
            <br />
            - 상태 관리 규칙 정의: - 컴포넌트 계층이 깊지 않을 경우 props /
            계층이 깊지만 복잡성이 낮거나 단순한 동작 제어인 경우 provide+inject
            / 컴포넌트 계층과 복잡성이 깊어질 경우 상태 관리 라이브러리(vuex)
            사용
            <br />
            ...
            81점으로 올림
            <br />- wav 음성 파일 재생 및 편집기 구현 (wavesurfer.js
            라이브러리): - 실시간으로 wav 데이터를 출력해 재생하고, 음성 파형에
            맞는 자막 element를 생성 및 동기화. 이를 통해 기존 텍스트 라벨링만
            하던 회사의 기술적 범위를 확장 시킴
          </div>
        </>
      }
      accordionContent={
        <>
          ...
          <p>
            → snackbar 컴포넌트는 app.config.globalProperties로 전역 설정 후
            외부 컴포넌트에서 getCurrentInstance 프로퍼티의 `proxy` 변수로 접근
            (global state 저장)
          </p>
          ...
          <p>→ code spliting 진행 → lcp/fid 개선</p>
          <p>
          ...
          <p>→ 텍스트 블럭 기능은 wavesurfer.js에서 제공하지 않음.</p>
          <p>
      ...
      content={
        <>
          <div>
            ...
            <br />
            - Next.js 서버/클라이언트 풀스택 구현: 프론트엔드와 백엔드를
            연결하는 전반적인 개발 프로세스를 경험
            <br />
            - excel 파일 db 업로드 기능 구현 (typeorm + psql)
            <br />- 초기에 개발자가 excel을 직접 전달 받아 db에 데이터를 insert
            하는 불편함 개선. 사용자가 직접 클라이언트에서 upload 가능하게끔 ui
            및 api 추가. 사용자와 개발자간 커뮤니케이션 비용 대폭 감소
          </div>
        </>
      }
      accordionContent={
        <>
          ...
          더욱 더 신경 써야 함! → React.js의 메모제이션을 활용 (**React.memo(),
          useCallback(), useMemo()**) → 제어 컴포넌트에서 비제어 컴포넌트로 변경
        </>
      }
    ></Article>
  );
      ...
      content={
        <>
          <div>
            ...
            서버 구축 (node.js)
            <br />- node.js 서버 와 firebase(firestore) 연동
          </div>
        </>
      }
      accordionContent={
      ...
      content={
        <>
          <div>
            ...
            <br />- 외부 api: openAi api(동화 생성) / deepai api(삽화 생성) 활용
            <br />- 배포 환경: 백엔드(Google cloudRun) + 프론트엔드(github
            Pages) <br />
            -webSocket으로 openai api 통신시 프론트엔드 로딩 화면 구현
          </div>
        </>
      }
      accordionContent={
        <p>
          ...
          필요한 에니메이션(페이지가 넘어가는 에니메이션)을 찾을수 없었음
          <br />
          삽화 생성 api 이슈: 삽화 생성 api가 계속 내용에 맞지 않는 엉터리
          그림을 생성함
        </p>
      }
    ></Article>
...
const ReactProgressBarArticle = (props) => {
  return (
    <Article
      ...
      subtitle={"React.js 오픈 소스 제작후 npm package로 배포"}
